...
def get_batch(split: str, data_dir: str, batch_size: int, block_size: int, device: str = "cuda"):
  """
...
class DataLoader:
  ...
  def data(self):
    ...
    if self._data is None:
      self._data = np.memmap(f"{self.data_dir}/tokenized/{self.split}.bin", dtype=np.uint16, mode="r")
    return self._data

  def get_batch(self):
    """Get a random batch of data."""
    return get_batch(self.split, self.data_dir, self.batch_size, self.block_size, self.device)
